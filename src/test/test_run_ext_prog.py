...
class TestRunExtProg(unittest.TestCase):
    ...
    def test_RunExtProg_append_reset_switch(self):
        prog1 = runExtProg("ls")
        self.assertEqual(prog1.get_extract_switch(), [])

        prog1.add_switch("-l")
        self.assertEqual(prog1.get_extract_switch(), ["-l"])

        prog1.add_switch("-s")
        self.assertEqual(prog1.get_extract_switch(), ["-l", "-s"])

        prog1.add_switch(["-1", "-2"])
        self.assertEqual(prog1.get_extract_switch(), ["-l", "-s", "-1", "-2"])

        prog1.add_switch(["-x"])
        self.assertEqual(prog1.get_extract_switch(), ["-l", "-s", "-1", "-2", "-x"])

        prog1.reset_switch()
        self.assertEqual(prog1.get_extract_switch(), [])

        prog1.add_switch(["-a", "-b"])
        self.assertEqual(prog1.get_extract_switch(), ["-a", "-b"])
    ...
    def test_RunExtProg_set_switch(self):
        ...
        prog1.set_switch("-s")
        self.assertEqual(prog1.get_extract_switch(), ["-s"])

        prog1.set_switch(["-111", "-222"])
        self.assertEqual(prog1.get_extract_switch(), ["-111", "-222"])
...
#        print "\noutput:\n",self.out
    ...
    def test_RunExtProg_update_switch(self):
        ...
        self.assertEqual(prog1.get_extract_switch(), ["-a", "1", "--b", "2"])
        prog1.update_switch("-a", "3")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "3", "--b", "2"])
        prog1.update_switch("-c", "4")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "3", "--b", "2", "-c", "4"])
        prog1.update_switch("-b", "5")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "3", "--b", "2", "-c", "4", "-b", "5"])
        prog1.update_switch("-C", "6")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "3", "--b", "2", "-c", "4", "-b", "5", "-C", "6"])
        prog1.update_switch("--b", "7")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "3", "--b", "7", "-c", "4", "-b", "5", "-C", "6"])

    def test_RunExtProg_toggle_switch(self):
        prog1 = runExtProg("ls")
        prog1.add_switch(["-a", "1"])
        prog1.toggle_switch("-t")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "1", "-t"])

        prog1.toggle_switch("-t")
        self.assertEqual(prog1.get_extract_switch(), ["-a", "1"])
