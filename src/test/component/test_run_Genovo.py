...
class TestRunGenovo(unittest.TestCase):
    ...
    def test_RunGenovo_init(self):
        ...
        self.assertListEqual(genovo.finalize.get_switch(), ["10", "all_reads_out.fa", infile_var+".dump.best"])
...
#   def_test_ClassName_mhatAreWeTestingHere(self)
    ...
    def test_RunGenovo_setCutoff(self):   
        infile_var="newname.fasta"
        genovo = RunGenovo(infile=infile_var, pdir = self.data_dir, noI=3, thresh=250)
        self.assertRaises(TypeError, genovo.setCutoff, 1.1)
        self.assertRaises(TypeError, genovo.setCutoff, -1)
