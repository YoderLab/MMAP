...
class HClust(object):
    ...
    def tree_cluster(self, mask=None, weight=None, transpose=0, method='m'):
        '''
        ...
        '''
        self.tree = treecluster(distancematrix=self.distMatrix, mask=mask,
                                weight=weight, transpose=transpose, method=method)
    ...
    def cal_dist(self, data, mask=None, weight=None, transpose=0, dist='e'):
        ...
        '''
        self.distMatrix = distancematrix(data, mask=mask, weight=weight,
                                         transpose=transpose, dist=dist)
...
#        record.data = dist
        ...
        print dir(record)
        genetree = record.tree_cluster(method='s')
        genetree.scale()
        exptree = record.tree_cluster(dist='u', transpose=1)
...
#        from hcluster import *
        from hcluster import pdist, linkage, squareform, dendrogram, np
